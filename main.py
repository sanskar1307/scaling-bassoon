...
if __name__ == '__main__':
    table = readTxt('./transactionTable.txt')
    ...
    for l in range(1, 4):
        if l == 1:
            ...
            table = get_filtered_t_table(
                trans_table=table, L=LL[(l, 1)], l=l, minSup=minSup)
        else:
            LL[(l, 1)] = get_large_1_itemsets(trans_table=table, l=l)
        for k in range(2, 4):
            candidate_set = get_candidate_set(LL[(l, k-1)], k)
            subsets = get_subset_support(
                Ck=candidate_set, table=table, minSup=minSup[l], l=l)
